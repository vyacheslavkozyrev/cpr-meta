# Data Model - Feedback Submission Collection
...
**IMPORTANT**: NO DATABASE CHANGES REQUIRED FOR THIS FEATURE

The backend API for Feedback Submission & Collection is **already fully implemented** with complete database schema. This Phase 3 plan focuses exclusively on **frontend UI implementation**. All database entities, relationships, indexes, and migrations already exist in production.

This document provides reference information about the existing data model for frontend developers implementing the UI.

**Database**: PostgreSQL 16 (already configured and deployed)
...
```mermaid
erDiagram
    feedback ||--o| goals : "optionally references"
    feedback ||--o| projects : "optionally references"
    feedback }o--|| employees_from : "from_employee"
    feedback }o--|| employees_to : "to_employee"
    feedback ||--o| feedback_requests : "optionally responds to"

    feedback {
        uuid id PK "Primary key"
        uuid goal_id FK "Goal reference (NULLABLE)"
        uuid project_id FK "Project reference (NULLABLE)"
        uuid from_employee_id FK "Feedback provider"
        uuid to_employee_id FK "Feedback recipient"
        text content "Feedback text (10-2000 chars)"
        int rating "Rating 1-5"
        uuid feedback_request_id FK "Request reference (NULLABLE)"
...
**Note**: This ERD shows the existing, deployed database schema. No changes are required for this feature implementation.

---

## ⚠️ NO DATABASE CHANGES REQUIRED

**This feature is FRONTEND-ONLY implementation.** The backend API and database schema are already complete and deployed. This document serves as **reference material** for frontend developers to understand the existing data model when implementing UI components.

### What Already Exists (Backend Complete):

- ✅ `feedback` table with all columns (id, goal_id, project_id, from_employee_id, to_employee_id, content, rating, feedback_request_id, created_at, updated_at)
- ✅ Foreign key constraints (goal_id → goals.id, project_id → projects.id, from/to_employee_id → employees.id, feedback_request_id → feedback_requests.id)
- ✅ Indexes for performance (idx_feedback_to_employee, idx_feedback_goal, idx_feedback_created_at, composite index on (to_employee_id, created_at))
- ✅ C# domain model (`src/CPR.Domain/Entities/Feedback.cs`)
- ✅ EF Core configuration with proper column mappings (snake_case)
- ✅ Database migrations already applied to production

### What Frontend Developers Need to Know:

1. **Feedback Entity Structure**: Feedback has goal, project, from_employee, to_employee references (all optional except employees)
2. **Rating Scale**: Integer 1-5 (enforced by database constraint)
3. **Content Length**: TEXT field, application enforces 10-2000 characters
4. **Timestamps**: created_at and updated_at managed automatically by database triggers
5. **Soft Delete**: Feedback cannot be deleted; if goal/project deleted, foreign keys set to NULL (ON DELETE SET NULL)

---

## Existing Entity Reference

### 1. Feedback (Already Implemented)

**Table Name**: `feedback`

**Description**: Stores feedback submitted from one employee to another regarding goals and projects

**Relationships**:

- Belongs to: `employees` (from_employee_id) - Provider of feedback
- Belongs to: `employees` (to_employee_id) - Recipient of feedback
- Optionally references: `goals` (goal_id) - Associated goal
- Optionally references: `projects` (project_id) - Associated project context
- Optionally references: `feedback_requests` (feedback_request_id) - Originating request
...
-- Indexes
CREATE INDEX idx*[table]\_user_id ON [table_name](user_id);
CREATE INDEX idx*[table]_status ON [table_name](status);
CREATE INDEX idx_[table]\_created_at ON [table_name](created_at DESC);

-- Trigger for updated*at
CREATE TRIGGER update*[table]\_updated_at
...
- Specification: `specifications/0005-[feature-name]/description.md`
- Implementation Plan: `specifications/0005-[feature-name]/implementation-plan.md`
- Endpoints: `specifications/0005-[feature-name]/endpoints.md`
